    ...
    <main>
        ...
        <ul class="p-20 rounded-md grid grid-cols-3 gap-10">
            <li class="bg-white rounded-md px-5 py-10 text-lg w-[30rem]">
                ...
                $sql = "SELECT * FROM registration LEFT JOIN file_task ON registration.email = file_task.email";
                $result = mysqli_query($conn, $sql);
                $row = mysqli_fetch_assoc($result);
                echo "<div class='flex justify-between'>";
                echo "<b>ID: " . $row["id"] . "</b><br>";
                $path=$_SESSION["upload"];                
                echo '<img src="' . $path . '" alt="uploaded Image" class="h-10 w-10 rounded rounded-full border">';
                echo "</div>";
                ...
                echo "unique Name: " . $row["unique_name"] . "<br>";
                ...
                <?php if (mysqli_num_rows($result) > 0): ?>
                    <?php while ($row = mysqli_fetch_assoc($result)): ?>
                        <li class="bg-white rounded-md px-5 py-10 text-lg w-[30rem]">
                            ...
                            echo "<div class='flex justify-between'>";
                            echo "<b>ID: " . $row["id"] . "</b><br>";                
                            echo '<img src="IMAGES/' . $row["image_name"] . '" alt="uploaded Image" class="h-10 w-10 rounded rounded-full border">';
                            echo "</div>";
                            echo "<b><h2>" . $row["name"] . "</h2></b><br>";
                            echo "Email: " . $row["email"] . "<br>";
                            echo "DOB: " . $row["dob"] . "<br>";
